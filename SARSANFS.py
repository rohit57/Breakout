...
for episode in range(episodes):
    ...
    for step in range(steps_max):
        ...
        if done == True:
            ...
            if old_state_value in Q_Table:
                if state_value not in Q_Table:
                    ...
                    Q_Table[old_state_value][action] = Q_Table[old_state_value][action] + lr * (reward + gamma * (Q_Table[state_value][new_action]) - Q_Table[old_state_value][action])
                else:
                    Q_Table[old_state_value][action] = Q_Table[old_state_value][action] + lr * (reward + gamma * (Q_Table[state_value][new_action]) - Q_Table[old_state_value][action])
            else:
                if state_value not in Q_Table:
        ...
        num_random = random.random()
        if num_random > epsilon:
            if state_value in Q_Table:
                new_action = Q_Table[state_value].index(max(Q_Table[state_value]))
            else:
        ...
        if old_state_value in Q_Table:
            if state_value not in Q_Table:
                ...
                Q_Table[old_state_value][action] = Q_Table[old_state_value][action] + lr * (
                    reward + gamma * (Q_Table[state_value][new_action]) - Q_Table[old_state_value][action])
            else:
                Q_Table[old_state_value][action] = Q_Table[old_state_value][action] + lr * (
                    reward + gamma * (Q_Table[state_value][new_action]) - Q_Table[old_state_value][action])
        else:
...
import pickle
output = open('finalSNfsr2.pkl', 'wb')
